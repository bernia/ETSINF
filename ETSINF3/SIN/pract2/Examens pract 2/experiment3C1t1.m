...
printf("# maxIt        a       b     E     k   Ete       E%%  I95inf%%  I95sup%%\n");
printf("#------ -------- ------- ----- ----- ----- -------- -------- --------\n");
maxIt=str2num("50 200");
for maxK=maxIt
	for a=as
		for b=bs
			[w,E,k]=perceptron(data(1:NTr,:),b,a,maxK); rl=zeros(M,1);
			for n=1:M rl(n)=ll(linmach(w,[1 te(n,1:D)]')); end
			[nerr mc]=confus(te(:,L),rl);
			m=nerr/M;
			s=sqrt(m*(1-m)/M);
			r=1.96*s;
			printf("%6d %8.1g %8.1g %5d %5d %5d %8.1f %8.1f %8.1f\n",
				maxK,a,b,E,k,nerr,100.0*m,100.0*max(m-r,0),100.0*(m+r));
		end
	end
end
