...
public class Indexador {
            ...
            while (libro.hasNext()) {
                ...
                for (int i = 0; i < words.length; i++) {
                    ...
                    if (esTermino(clave)) {
                        ...
                        ListaConPI<Indice> lista = indices.recuperar(clave);
                        if(lista == null) {
                            lista = new LEGListaConPI();
                        }
                        lista.insertar(ind);
                        indices.insertar(clave,lista);
                        t2 = System.nanoTime();
                    }
                    tmp += (t2 - t1);
                }
    ...
    public ListaConPI<String> indiceDe(String pal) {
        ...
        ListaConPI<Indice> llista = indices.recuperar(pal);
        llista.inicio();
        while(!llista.esFin()) {
            res.insertar(llista.recuperar().toString());
            llista.siguiente();
        }
        return res;
    }
