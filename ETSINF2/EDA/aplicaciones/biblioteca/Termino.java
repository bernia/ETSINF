...
public class Termino {
    ...
    private int hashSimple() { 
        int res = 0;
        for(int i = 0; i < termino.length(); i++) { res += termino.charAt(i); }
        return res;
    }
    ...
    private int hashWeiss() { 
        int res = 0;
        int n = termino.length() - 1;
        int exp = 1;
        
        for (int i = 0; i < termino.length(); i++) {
            exp = 1;
            for (int j = n; j > 0; j--) { exp *= 37; }
            res += termino.charAt(i) * exp;
            n--;
        }
        return res;
    }
    ...
    public boolean equals(Object o) {
        return (o instanceof Termino) && this.termino.equals(((Termino) o).termino)
        && this.valorHash == ((Termino) o).valorHash
        && this.fHash == ((Termino) o).fHash;
    }
