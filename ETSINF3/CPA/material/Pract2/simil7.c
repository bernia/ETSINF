...
{ int i,j,d,im,dm, lmin, lmax, cont;// dm es la distancia del código más parecido, im su índice
  double lmitjana,mitjanaParcial; // dm is the distance of the most similar code, im is its index
  lmin = 999999999;
  lmax = 0;
  lmitjana = 0;
  cont = 0;
  mitjanaParcial = 0;
	#pragma omp parallel private(i,dm,d,im,mitjanaParcial) reduction(+:lmitjana,cont),reduction(min:lmin),reduction(max:lmax)
	{
		#pragma omp for 
		  for ( j = 0 ; j < nt2 ; j++ ) {
			dm = MAX_LON + 1;
		...
		mitjanaParcial = lmitjana/cont;
		printf("Fil %d de %d : Longitud min/mitja/max: %d/%6.2f/%d\n", omp_get_thread_num(),
			omp_get_num_threads(), lmin, mitjanaParcial, lmax);
	}
	lmitjana = lmitjana/cont;
	printf("TOTAL: Longitud min/mitja/max: %d/%6.2f/%d\n", lmin, lmitjana, lmax);
}
