...
public class GrafoRedMunicipios {
    ...
    public double crearAdyKruskal() {      
        ...
        Arista[] solucio = gRM.kruskal();
        int sumaPes = 0;
        // PASO 2. Si kruskal devuelve null, devolver -1.0
        if(solucio == null) { return -1.0; }
        ...
        for(int i = 0; i < solucio.length; i++) {
            sumaPes += solucio[i].getPeso();
            Municipio m = verticesAMunicipios.recuperar(solucio[i].getOrigen());
            ListaConPI<Municipio> l = adyKruskal.recuperar(m);
            if (l == null) { l = new LEGListaConPI(); }
            l.insertar(verticesAMunicipios.recuperar(solucio[i].getDestino()));
            adyKruskal.insertar(m,l);
