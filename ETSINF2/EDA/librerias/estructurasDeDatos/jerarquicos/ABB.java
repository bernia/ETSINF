...
public class ABB<E extends Comparable<E>> {
    ...
    public ABB(E[] v) {
        int m = v.length/2;
        raiz = new NodoABB(v[m],construirEquilibrado(v,0,m-1),
            construirEquilibrado(v,m+1,v.length-1));
    }
    ...
    protected NodoABB<E> construirEquilibrado(E[] v, int ini, int fin) {
        ...
        //if (ini == fin) { return new NodoABB(v[ini]); }
        else {
            int m = (fin+ini)/2;
            NodoABB<E> res = new NodoABB(v[m],construirEquilibrado(v,ini,m-1),
                construirEquilibrado(v,m+1,fin));
            return res;
        }
    }
    ...
    protected NodoABB<E> sucesor(E e, NodoABB<E> actual) {
        ...
        if (e.compareTo(actual.dato) < 0) { 
            NodoABB<E> suc = sucesor(e,actual.izq);
            if (suc == null) {
                return actual;
            } else {
                return suc;
