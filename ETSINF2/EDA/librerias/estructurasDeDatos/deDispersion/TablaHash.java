...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    public V recuperar(C c) {
        ...
        //Resolucion de la Busqueda: SII ablaHasesta la Entrada se recupera su valor
        if (!l.esFin()) {
    ...
    public ListaConPI<C> claves() {
        ListaConPI<C> claus = new LEGListaConPI();
        for(int cub = 0; cub < elArray.length; cub++){
            ListaConPI<EntradaHash<C, V>> aux = elArray[cub];
            aux.inicio();
            while (!aux.esFin()){
                claus.insertar(aux.recuperar().clave);
                aux.siguiente();
            }
        }
        return claus;
    ...
    protected final void rehashing() {
        ListaConPI<EntradaHash<C, V>>[] elArrayVell = elArray;
        ...
        talla = 0;
        for (int i = 0; i < elArrayVell.length; i++) { 
            ListaConPI<EntradaHash<C, V>> aux = elArrayVell[i];
            aux.inicio();
            while (!aux.esFin()){
    	        insertar(aux.recuperar().clave,aux.recuperar().valor);
    	        aux.siguiente();
    	    }
        }
    ...
    public final double desviacionTipica() {
    	...
    	for(int cub=0; cub < elArray.length; cub++){
    	    ...
    	    sumaQuadrats += (mitjana - contElemCub)*(mitjana - contElemCub);
    	}
    
    	sumaQuadrats /= elArray.length;
