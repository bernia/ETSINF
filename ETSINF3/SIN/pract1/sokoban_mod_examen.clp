; Javier Martinez Bernia
(defglobal ?*NODE* = 0)
(defglobal ?*L* = 10)
(deffacts bf (map 3 4)(obs 1 1) (obs 2 1) (obs 1 4) (obs 2 4)
(S 3 1 C 2 3 C 3 2 M 3 3 0 M 3 4 0 N 0))
...
(defrule dretaEMM1
	(declare (salience 3))
	(S ?f ?c $?z M ?f ?cm1 1 $?z1 M ?f ?cm2 0 $?z2 N ?n)
	(test (= ?c (- ?cm1 1)))
	(test (= ?c (- ?cm2 2)))
	=>
	(assert (S ?f (+ ?c 1) $?z M ?f ?cm1 0 $?z1 M ?f ?cm2 1 $?z2 N (+ ?n 1))))
	
(defrule dretaEMM2
	(declare (salience 3))
	(S ?f ?c $?z M ?f ?cm1 0 $?z1 M ?f ?cm2 1 $?z2 N ?n)
	(test (= ?c (- ?cm1 2)))
	(test (= ?c (- ?cm2 1)))
	=>
	(assert (S ?f (+ ?c 1) $?z M ?f ?cm2 0 $?z1 M ?f ?cm1 1 $?z2 N (+ ?n 1))))
...
(defrule acaba
	...
	(S ? ? M $? N ?n)
	=> 
	(printout t "Solucio trobada amb nivell de profunditat " ?n crlf)
