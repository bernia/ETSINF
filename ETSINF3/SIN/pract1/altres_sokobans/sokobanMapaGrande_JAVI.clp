...
(defrule caixaAvall
	(S ?f ?c $?z C ?fc ?cc $?z1) (map ?maxF ?maxC)
	(test (< ?f =(- ?maxF 1)))
	(not obs =(+ ?f 2) ?c)
	(test (= ?f =(- ?fc 1)))
	(test (= ?c ?cc))
...
(defrule caixaAvallMagatzem
	(S ?f ?c $?z C ?fc ?cc $?z1 M ?fm ?cm ?e $?z2) (map ?maxF ?maxC)
	(test (< ?f =(- ?maxF 1)))
	(not obs =(+ ?f 2) ?c)
	(test (= ?f =(- ?fc 1)))
	(test (= ?c ?cc))
...
(defrule caixaEsquerra
	...
	(test (= ?f ?fc))
	(test (= ?c =(+ ?cc 1))
	(test not((member$ (create$ M ?f =(- ?c 1)) $?z1)))
	=>
	(assert (S ?f (- ?c 1) $?z C ?fc (- ?cc 1) $?z1))
...
(defrule caixaEsquerraMagatzem
	...
	(test (= ?e 0))
	=>
	(assert (S ?f (- ?c 1) $?z C ?fc (- ?cc 1) $?z1 M ?fm ?cm (+ ?e 1) $?z2))
